...
manifests: controller-gen
	...
	$(CONTROLLER_GEN) $(CRD_OPTIONS) paths="./vendor/volcano.sh/apis/pkg/apis/batch/v1alpha2" output:crd:artifacts:config=docs/deploy
	# generate hyperjob crd without description to avoid yaml size limit when using `kubectl apply`
	$(CONTROLLER_GEN) $(CRD_OPTIONS_EXCLUDE_DESCRIPTION) paths="./vendor/volcano.sh/apis/pkg/apis/batch/v1alpha2" output:crd:artifacts:config=docs/deploy
	rm -rf vendor
