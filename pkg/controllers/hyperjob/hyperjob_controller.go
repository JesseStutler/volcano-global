...
func (h *HyperJobController) constructConditions(replicatedJobStatus []trainingv1alpha1.ReplicatedJobStatus, splitCount int32) []metav1.Condition {
	...
	// HyperJob is still in progress if not all child jobs are finished
	if finishedJobs != splitCount {
		condition.Status = metav1.ConditionFalse
		condition.Reason = HyperJobReasonIsInProgress
		condition.Message = fmt.Sprintf("HyperJob in progress: %d/%d finished (%d completed, %d failed, %d aborted, %d terminated), %d running, %d pending, %d in other transition states",
			finishedJobs, splitCount, completed, failed, aborted, terminated, running, pending, other)
		return []metav1.Condition{condition}
	}

	condition.Status = metav1.ConditionTrue
	condition.Reason = HyperJobReasonCompleted
	if failed+aborted+terminated == 0 {
		condition.Message = fmt.Sprintf("All %d jobs completed successfully", splitCount)
	} else {
		condition.Message = fmt.Sprintf("HyperJob completed: %d succeeded, %d failed, %d aborted, %d terminated",
